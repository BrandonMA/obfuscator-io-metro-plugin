 ...
 var offset = { line: 2 };
  ...
  postData.Items.map((item)=>
  {
    try{
      ...
      const numberOfLines = item.FileCode.split('\n').length;
      sourceMaps = sourceMaps.addFile({source:obfuscationResult.getSourceMap(), sourceFile:item.FileName}, {
        line: offset.line + numberOfLines
      });
    }
    catch(error){
